...
function generateWiresModule(serial) {
  ...
  return { type: 'wires', wires, solution, solved: false };
}
...
function createRoom(roomId) {
  ...
  rooms[roomId] = {
    id: roomId, players: {}, serial, modules,
    strikes: 0, maxStrikes: 3, timeLeft: 300,
    started: false, gameOver: false, won: false,
    timer: null, chatHistory: [],
  };
  return rooms[roomId];
}
...
function availableRole(room) {
  ...
  return ['bomber', 'expert', 'relay'].find(r => !taken.includes(r));
}
...
function getManualData(room) {
  return {
    ...
    modules: room.modules.map(m => {
      if (m.type === 'wires') return { type: 'wires', wires: m.wires, solution: m.solution, solved: m.solved };
      if (m.type === 'button') return { type: 'button', color: m.color, label: m.label, batteries: m.batteries, solution: m.solution, solved: m.solved };
      if (m.type === 'simon') return { type: 'simon', sequence: m.sequence, solved: m.solved };
      return m;
    }),
  };
}
...
function getBombData(room) {
  return {
    ...
    modules: room.modules.map((m, i) => {
      ...
      if (m.type === 'button') return { type: 'button', index: i, color: m.color, label: m.label, batteries: m.batteries, solved: m.solved };
...
function broadcastState(room) {
  Object.entries(room.players).forEach(([sid, player]) => {
    const base = {
      strikes: room.strikes, maxStrikes: room.maxStrikes, timeLeft: room.timeLeft,
      gameOver: room.gameOver, won: room.won,
...
function checkWin(room) {
  if (room.modules.every(m => m.solved)) {
    room.gameOver = true; room.won = true;
    if (room.timer) clearInterval(room.timer);
    broadcastState(room);
    io.to(room.id).emit('gameEnd', { won: true, message: 'BOMBE DESAMORCEE ! Felicitations !' });
  }
}

function triggerExplosion(room, reason) {
  room.gameOver = true; room.won = false;
  if (room.timer) clearInterval(room.timer);
  broadcastState(room);
...
function startTimer(room) {
  ...
  room.timer = setInterval(() => {
    ...
    if (room.timeLeft <= 0) triggerExplosion(room, 'TEMPS ECOULE ! La bombe a explose !');
  }, 1000);
}
...
io.on('connection', (socket) => {
  socket.on('createRoom', ({ name }) => {
    ...
    room.players[socket.id] = { id: socket.id, name, role: 'bomber' };
    socket.join(roomId);
    socket.emit('roomCreated', { roomId, role: 'bomber', name });
  ...
  socket.on('joinRoom', ({ roomId, name }) => {
    ...
    if (Object.keys(room.players).length >= 3) { socket.emit('error', { message: 'Salle pleine.' }); return; }
    if (room.started) { socket.emit('error', { message: 'Partie deja en cours.' }); return; }
    ...
    if (Object.keys(room.players).length === 3) {
      room.started = true;
      setTimeout(() => { startTimer(room); broadcastState(room); io.to(roomId).emit('gameStarted', {}); }, 1000);
    }
  });
  ...
  socket.on('reconnectGame', ({ roomId, role, name }) => {
    ...
    if (existing) delete room.players[existing.id];
  ...
  socket.on('chatMessage', ({ text }) => {
    ...
    addChat(room, { sender: player.name, role: player.role, text, type: 'chat', timestamp: Date.now() });
  });
  ...
  socket.on('expertSignal', ({ signal }) => {
    ...
    const player = room.players[socket.id];
    if (!player || player.role !== 'expert') return;
    addChat(room, { sender: player.name, role: 'expert', text: signal, type: 'signal', timestamp: Date.now() });
  });
  ...
  socket.on('action', ({ moduleIndex, action, data }) => {
    ...
    if (mod.type === 'wires') {
      ...
      if (cut === mod.solution) { mod.solved = true; msg = `Fil ${cut + 1} coupe - Module FILS desamorce !`; }
      else { room.strikes++; msg = `Mauvais fil ! Strike ${room.strikes}/${room.maxStrikes}`; }
    }
    if (mod.type === 'button') {
      if (action === 'press') {
        if (mod.solution === 'press') { mod.solved = true; msg = 'Bouton appuye - Module BOUTON desamorce !'; }
        else { room.strikes++; msg = `Il fallait maintenir ! Strike ${room.strikes}/${room.maxStrikes}`; }
      } else if (action === 'holdRelease') {
        if (mod.solution === 'hold') { mod.solved = true; msg = 'Bouton maintenu - Module BOUTON desamorce !'; }
        else { room.strikes++; msg = `Il fallait appuyer ! Strike ${room.strikes}/${room.maxStrikes}`; }
      }
    }
    if (mod.type === 'simon') {
      const translated = getSimonTranslation(mod.sequence[mod.current], room.strikes);
      if (data.color === translated) {
        mod.current++;
        if (mod.current >= mod.sequence.length) { mod.solved = true; msg = 'Sequence complete - Module SIMON desamorce !'; }
        else { msg = `Bonne couleur ! Etape ${mod.current}/${mod.sequence.length}`; broadcastState(room); addChat(room, { sender: 'SYSTEME', role: 'system', text: msg, type: 'system', timestamp: Date.now() }); return; }
      } else { mod.current = 0; room.strikes++; msg = `Mauvaise couleur, reinitialise ! Strike ${room.strikes}/${room.maxStrikes}`; }
    }
    addChat(room, { sender: 'SYSTEME', role: 'system', text: msg, type: 'system', timestamp: Date.now() });
    broadcastState(room);
    if (room.strikes >= room.maxStrikes) { triggerExplosion(room, 'TROP D ERREURS ! La bombe a explose !'); return; }
    checkWin(room);
  });
  ...
  socket.on('simonFlash', ({ moduleIndex }) => {
    ...
    io.to(room.id).emit('simonFlash', { moduleIndex, color: mod.sequence[mod.current], step: mod.current, total: mod.sequence.length, strikes: room.strikes });
  });
  ...
  socket.on('disconnect', () => {
    ...
    if (!room) return;
    const player = room.players[socket.id];
    if (player && room.started && !room.gameOver) triggerExplosion(room, `${player.name} a quitte. La bombe a explose !`);
    delete room.players[socket.id];
    if (Object.keys(room.players).length === 0) { if (room.timer) clearInterval(room.timer); delete rooms[room.id]; }
    else io.to(room.id).emit('playerList', Object.values(room.players));
  });
  ...
  socket.on('restartGame', () => {
    ...
    setTimeout(() => { startTimer(newRoom); broadcastState(newRoom); io.to(room.id).emit('gameStarted', {}); }, 1000);
  });
});

const PORT = process.env.PORT || 3000;
server.listen(PORT, () => console.log(`Bomb Defusal running on port ${PORT}`));
